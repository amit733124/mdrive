...
<div class="content">
  <i class="fa fa-history" style="font-size: 40px;color: #2A3F54;margin-bottom: 10px;margin-left: 0px">Last Activity/</i><br>
  <form action="" method="POST">
<fieldset style="background:rgba(0,0,255,.3);height:auto;width:200px;">
  <?php
require("conn.php");
...
require("conn.php");
$pd=$_POST['pd'];
$sql="select * from user_table where email='$pd' ";
$record=mysqli_query($con,$sql);
echo "<table style='border:1px solid #ddd;background-color:#F1F1F1'>";
echo "<tr style='background-color:green;color:white'><th>Name</th><th>Username</th><th>Email</th><th>Mobile_No</th><th>IP</th><th>Date_Time</th><th>Operation</th></tr>";
while($row=mysqli_fetch_array($record))
{
echo "<tr>";
echo "<td>".$row[0]."</td>";
echo "<td style='color:red;font-family:bold'>".$row[1]."</td>";
echo "<td>".$row[8]."</td>";
echo "<td>".$row[9]."</td>";
echo "<td>".$row[12]."</td>";
echo "<td>".$row[13]."</td>";
...
?>
<td><a href="del_active_user?id=<?php echo $row[1]; ?>" onclick="return confirm('Are you sure to Delete ?');"><i class="fa fa-trash"> Delete</a></i>&nbsp<a href="limit?id=<?php echo $row[1]; ?>" onclick="return confirm('Are you sure to Delete ?');"><i class="fa fa-line-chart"> Limit</a></td>
  <?php
echo "</tr>";
}
echo "</table>";
